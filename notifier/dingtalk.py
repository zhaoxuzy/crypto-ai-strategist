...
def format_strategy_message(symbol: str, strategy: dict, current_price: float, extra: dict) -> str:
    ...
    if trend_direction == "bull":
        market_state = "上涨趋势" if trend_score >= 70 else ("震荡偏强" if trend_score >= 30 else "弱势震荡")
    elif trend_direction == "bear":
        market_state = "下跌趋势" if trend_score >= 70 else ("震荡偏弱" if trend_score >= 30 else "弱势震荡")
    else:
        market_state = "无明显方向"
    if 30 <= trend_score <= 70:
        market_state += "（方向不明）"

    if direction == "long": dir_display = "做多"
    elif direction == "short": dir_display = "做空"
    else: dir_display = "观望"
