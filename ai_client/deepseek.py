...
def build_prompt(data: dict, symbol: str) -> str:
    ...
    prompt = f"""你正在用自己管理的200万U资金进行真实的交易决策。你不是在答题，你是在为盈亏负责。使用第一人称“我”进行所有思考。
...
---
【硬性约束】
1. 必须且只能引用上方提供的具体数据，不得编造任何数值。
2. 你的 `reasoning` 字段必须是一个完整的、自包含的推演文本，它必须包含每一步的“分析数据”、“第一反应”、“自我质疑”、“最终结论”子标题及其详细内容，你的思考过程必须显式地写出来，不得简化或跳过。

---
第一步：环境定调
分析数据：价格7日分位数、1h ATR、波动因子。
第一反应：
自我质疑：
最终结论：
...
第六步：矛盾裁决与决策
请基于前五步的所有数据和自己的内心对峙，独立做出最终决策。

1. 【信号传唤】
   - 明确指出前五步中，哪一个信号是你见过最强的看涨证据，它来自哪一步？
   - 明确指出前五步中，哪一个信号是你见过最强的看跌证据，它来自哪一步？

2. 【权重审判】
   - 为你做最终裁决所依据的关键决策因子分配权重（合计100%）。
   - 必须简要说明：在当前的市场阶段下，为什么会赋予某些因子更重的权重？

3. 【心证交锋】
   - 你必须模拟你的内心对话：扮演一个与你看反方向的头寸与你辩论，用最强反向证据攻击你的逻辑。
   - 然后，你再以主交易员身份，驳斥或接纳这个攻击。
   - 最终解释，你为何在当前的特定时间、特定价位下，选择压过最强反向信号的干扰。若你被反向证据说服，必须立即选择观望。

4. 【核心假设与证伪条件】
   - 清晰写出你判断方向的核心逻辑链：“因为[关键假设A]，所以[现象B]，进而[行情C]”。
   - 设定你的“逃生门”：明确价格行为（如1H收盘站上X）和资金流确认（如CVD转向），两者须共同触发，你才承认失败。

【最终输出】
方向选择：
置信度：
仓位：
流动性猎杀推演（必须专业研判，基于当前清算池分布、对手盘结构和资金流方向，描述价格最可能如何测试并触发关键流动性区域及连锁反应，需包含触发条件和证伪标准）：
...
{{
  ...
  "reasoning": "完整的六步推演内容，必须包含流动性猎杀推演",
  "risk_note": "风险说明"
}}
"""
    return prompt
