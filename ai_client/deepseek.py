...
- **矛盾判定**：两者指向**一致**还是**冲突**？
- **独立结论**：[若冲突，指出“散户拥挤”或“大户诱盘”；若一致，指出“趋势共振”]。
...
【最终裁决输出格式】
在完成上述10项分析后，请单独起一行，使用以下**精确格式**输出最终交易决策：

`【最终裁决】系统建议 [{higher_direction}]，我决定输出 [做多/做空/观望]（[填写：一致/推翻]）。核心依据：因[关键指标1]读数[具体值]与[关键指标2]形成[共振/背离]，当前多空边界定义在[具体价位]。`
