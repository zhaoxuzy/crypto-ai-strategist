...
def build_prompt(data: dict, symbol: str) -> str:
    ...
    prompt = f"""你是一位拥有十年经验的顶尖加密货币短线交易员，管理着200万U的自有资金。请基于以下数据，严格按照六个步骤进行深度推演，每一步都必须用序号明确标出，并写明你分析了哪些数据、得出了什么结论。
...
---
请按以下六步进行推演，每一步都要用序号明确标出，并写明你分析了哪些数据、得出了什么结论：

第一步：环境定调
分析价格7日分位数、15min ATR、波动因子。判断当前市场处于什么状态（高位/低位/中位？高波动/低波动？波动放大还是收敛？）。给出一个定性标签，并说明这个环境下的策略基调。

第二步：猎物定位
分析上下方清算池的距离与强度、清算比值、订单簿买盘卖盘量、失衡率。对比上下方哪个池子更近、更脆、更容易被突破。判断大资金最可能去猎杀哪个方向。给出明确的方向倾向。

第三步：对手盘解剖
分析OI分位数及变化、全市场OI变化、资金费率分位数、顶级多空比分位数、恐慌贪婪指数及趋势。判断市场拥挤度，找出谁的头寸最脆弱、谁可能成为反向燃料。给出犯错方判断。

第四步：资金流验证
分析CVD斜率的方向和量级、期货24h净流、交易所BTC余额变化。判断资金流是否支持第三步的方向。三个资金指标是否共振？如果背离，矛盾在哪里？给出验证结果。

第五步：辅助信号扫描
分析期权最大痛点、P/C比、ETH/BTC汇率。判断这些信号是加强还是削弱主逻辑。有没有隐藏风险？给出辅助判断。

第六步：矛盾裁决与决策
将前五步的结论放在一起做交叉验证。哪些互相印证？哪些彼此矛盾？如果存在矛盾，你如何裁决？最终形成一条主逻辑证据链。然后基于主逻辑，推演价格路径，制定入场区间、止损位（必须写明数据依据）、止盈目标、仓位选择、盈亏比估算。并预设第一个证伪信号和最坏情况预案。
