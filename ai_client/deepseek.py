...
【价格路径推演 - 三段式】（必须包含）
结合清算动力学（清算池位置与强度）、猎物定位（哪类头寸最脆弱）、犯错方（谁在亏损、谁在盈利）等条件，用“先……然后……最后……”的结构详细描述接下来价格最可能的演变路径。每段必须包含：
- 具体的价格目标（精确到个位数）
- 触发该段运动的盘口信号或条件（如：卖一墙厚度被消耗至X万、CVD斜率转正/转负、特定时间窗口）
- 预计持续时间或关键时间节点（如：5分钟内脉冲、15分钟级别盘整）
