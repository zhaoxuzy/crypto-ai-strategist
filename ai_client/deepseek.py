...
推演与决策：
1. 推演价格最可能路径。你必须详细写出三段式演变过程，格式如下：
   - 第一段（启动）：从当前价格开始，最先朝哪个方向移动？触发条件是什么（例如突破某个清算墙或均线）？会先猎杀哪一侧的止损？
   - 第二段（反应）：到达第一目标后，市场会如何反应？回踩到哪里？震荡区间？哪些数据支撑这一判断？
   - 第三段（终局）：最终价格会走向何处？可能触及的第二个目标、最大痛点或流动性池。
   请用“先...然后...最后...”的叙事结构，并明确写出关键价格点位。
