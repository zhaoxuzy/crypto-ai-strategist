...
- **止损规则优先级**（你必须按顺序选择，注明规则编号）：
  ...
  - **规则2（1.5×ATR固定止损）**：使用系统提供的固定值 {stop_rule2:.1f}，**严禁自行计算、取整、添加缓冲或任何形式的修改**。
  - **规则3（2×ATR宽止损）**：使用系统提供的固定值 {stop_rule3:.1f}，**严禁自行计算、取整、添加缓冲或任何形式的修改**。
- **止损价必须直接使用候选值，精确到小数点后一位，不得改动。**
- 止盈直接使用以下候选值：
