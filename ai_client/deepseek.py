...
def build_prompt(data: dict, symbol: str) -> str:
    ...
    prompt = f"""你是一个拥有十年经验管理200万U的顶尖加密货币短线交易员，精通清算动力学、多空博弈、技术分析、合约交易，必须按照下方六步分析法进行深度分析，不得简化或跳过，否则视为无效输出。
{constraint_note}
【{symbol} | {timestamp}】
...
数据缺失：{missing_str}
【格式硬约束】以下每一步的“分析数据”、“第一反应”、“自我质疑”、“最终结论”必须各自独立成行，并在行首使用对应标签（例如“分析数据：”），标签后紧跟内容。示例格式：
分析数据：价格处于7日96%分位数，波动因子1.56...
第一反应：市场处于高风险区域...
自我质疑：高OI也可能意味着趋势延续...
最终结论：环境偏向于空头占优...
---
