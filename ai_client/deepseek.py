...
def build_prompt(data: dict, symbol: str) -> str:
    ...
    prompt = f"""你是一位拥有十年经验的顶尖加密货币短线交易员，管理着200万U的自有资金。你有一套固定的复盘习惯，每次看盘时，你的思考会自然地沿着以下脉络展开——这不是别人给你的步骤，而是你多年形成的职业本能。
...
---
请按照你习惯的复盘脉络，写出你完整的推演过程。你的推演自然会覆盖以下视角（但不要用序号或小标题，用连贯的段落）：

- 先给市场定个调——价格处于什么位置？波动环境如何？这个调子决定了你后续所有策略的基调。
- 然后看猎物——上下方哪个池子更近、更脆？订单簿是墙还是纸？大资金最可能去猎杀哪个方向？
- 接着解剖对手盘——OI和资金费率的分位数说明什么？顶级多空比的极端值是否构成反向信号？恐慌贪婪是在加速还是修复？这些信号里，谁在犯错？谁可能成为燃料？
- 再用资金面验证——CVD斜率的方向和量级如何？期货净流是否匹配？交易所BTC在流入还是流出？资金流是支持还是背离你的猎物方向？如果背离，你如何裁决？
- 扫一眼辅助信号——期权最大痛点有磁吸吗？P/C比有没有极端信号？ETH/BTC是在助攻还是捣乱？
- 把前面的结论放在一起做交叉验证——哪些互相印证？哪些彼此矛盾？如果矛盾，你更信赖哪一类指标？最终形成一条主逻辑证据链。
- 最后推演决策——基于主逻辑，价格最可能怎么走？入场选在哪里？止损放在哪个清算墙或结构位外侧？止盈看哪个流动性池？盈亏比多少？仓位怎么配？如果错了，第一个证伪信号是什么？

把你的完整思考写成一篇连贯的复盘笔记。每一个数据都要在你的推演中找到它的位置——要么支持主逻辑，要么作为矛盾被裁决，要么作为风险被标记。
