...
**【数据使用原则】**
系统定性标签仅供参考。分析时必须优先信任原始序列。若标签与序列趋势不符，以序列为准，若数据缺失请注明。
...
**最终裁决要求**：
以一个顶级交易员的角色综合分析、全面研判后单独一行写【最终裁决】段落：
`【最终裁决】系统建议 [{higher_direction}]，我决定输出 [做多/做空/观望]（若与系统一致，写“一致”；若相反，写“推翻”）。核心依据：...`
