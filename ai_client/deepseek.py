...
**第五步：止损与止盈设置**
- **你必须根据清算热力图结构自行设定止损和止盈**，并严格遵守方向约束。
- **止损锚定原则**：
...
- **止盈锚定原则（严格遵守方向约束）**：
  - **做多时**：
    - TP1：优先锚定**上方**最近强度≥3的空头清算密集区。若无，则使用 1.0~1.5×ATR 灵活设定，并在描述中注明“无明确上方清算区，采用ATR估算”。
    - TP2：优先锚定**上方**更远的空头清算密集区或**高于入场价**的清算最大痛点。若均不存在，则使用 2.5~3.5×ATR 设定。**严禁使用低于入场价的目标作为止盈**。
  - **做空时**：
    - TP1：优先锚定**下方**最近强度≥3的多头清算密集区。若无，则使用 1.0~1.5×ATR 灵活设定，并在描述中注明“无明确下方清算区，采用ATR估算”。
    - TP2：优先锚定**下方**更远的多头清算密集区或**低于入场价**的清算最大痛点。若均不存在，则使用 2.5~3.5×ATR 设定。**严禁使用高于入场价的目标作为止盈**。
- **在reasoning中必须明确写出止损和每个止盈的锚定来源，并确保描述与方向一致**（如做多时TP1描述为“上方空头清算区”或“ATR估算”）。
